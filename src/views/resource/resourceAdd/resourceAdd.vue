        ...
        <div>
          ...
          >
            ...
            >
              ...
              <div
                class="price"
                v-if="this.form.payment_type==true"
              >
                <el-input
                  placeholder=""
                  v-model="form.pay_bodou"
                >
      ...
      if (this.edit === 0) {
        Resource.ResourcePackage.addResource(postObj)
          .then((rec) => {
            ...
            setTimeout(() => { this.$router.push({ path: `/resource/myResource/${rec.id}?type=course` }); }, 1500);
          })
          .catch((err) => {
            console.log('err=', err);
            this.$message({
